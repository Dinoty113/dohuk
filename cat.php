...
SELECT * FROM ".$Prefix."CATEGORY WHERE
...
<tr><td class=cat_new colspan=2>'.$lang['others']['members_now'].'</td></tr><tr><td style="font-size:12px;"><center>'.$lang['home']['members'].' <font color=red>'.$members_online.'</font><br>'.$lang['home']['the_moderator'].' <font color=red>'.$moderators_online.'</font>';if (mlv > 2){echo'<br>'.$lang['home']['the_monitor'].' <font color=red>'.$monitors_online.'</font>';echo'<br>'.$lang['members']['deputy_monitor'].': <font color=red>'.$deputy_monitors_online.'</font>';}echo'<br>'.$lang['home']['visitor'].' <font color=red>'.$visitors_online.'</font></table></td><td width=99% valign=top><table dir="rtl" border=0 width=100% cellspacing=1 cellpadding=0 class=maingrid><table dir="rtl" border=0 width=100% cellspacing=1 cellpadding=0 class=maingrid>	<center>	<table class="grid" cellSpacing="1" cellPadding="1" width="100%" border="0" dir=rtl>';		if ($Mlevel == 4){	echo'					<tr>					<td class="cat_new" align="middle" colspan=11>						<a href="index.php?mode=add_cat_forum&method=add&type=c">'.icons($folder_new, $lang['home']['add_new_cat'], "hspace=\"3\"").'</a>						<a href="index.php?mode=order">'.icons($folder_new_order, $lang['home']['order_cat_and_forum'], "hspace=\"3\"").'</a>						<a href="index.php?mode=other_cat_add&method=cat">'.icons($folder_other_cat, $lang['other_cat_forum']['add_cat'], "hspace=\"3\"").'</a>						<a href="index.php?mode=ihdaa_add&method=forum">'.icons($folder_other_forum, $lang['ihdaa']['active'], "hspace=\"3\"").'</a>					</td>				</tr>';	}
			$cat = DBi::$con->query("SELECT * FROM ".$Prefix."CATEGORY WHERE CAT_ID = '$c' AND (SITE_ID = '$Site_ID' OR SITE_ID = '0') ORDER BY CAT_ORDER ASC") or die (DBi::$con->error); // category mysql
			$c_num = $cat->num_rows;
			if ($c_num <= 0) {
				echo'
				<tr>
...
if($chakl == 0){
					...
					echo'
					<tr>
											<td class="cat_new" width="5%"><nobr><a href="index.php?mode=c&c='.$c.'">'.icons($folder).'</a></nobr></td>
                       ...
                       echo '<td class="cat_new"><nobr><a href="index.php?mode=admin_svc&type=forumsorder">'.icons($icon_arrowup).icons($icon_arrowdown).'</a></nobr></td>';
						...
						if ($Mlevel == 4) {
							...
							<td class="cat_new" vAlign="middle" align="middle"><nobr>
								<a href="index.php?mode=add_cat_forum&method=add&type=f&c='.$c.'">'.icons($folder_new, $lang['home']['add_new_forum'], "hspace=\"3\"").'</a>
								<a href="index.php?mode=add_cat_forum&method=edit&type=c&c='.$c.'">'.icons($folder_new_edit, $lang['home']['edit_cat'], "hspace=\"3\"").'</a>';
							if ($cat_status == 1) {
								echo'
								<a href="index.php?mode=lock&type=c&c='.$c.'"  onclick="return confirm(\''.$lang['home']['you_are_sure_to_lock_this_cat'].'\');">'.icons($folder_new_locked, $lang['home']['lock_cat'], "hspace=\"3\"").'</a>';
							}
							if ($cat_status == 0) {
								echo'
								<a href="index.php?mode=open&type=c&c='.$c.'"  onclick="return confirm(\''.$lang['home']['you_are_sure_to_open_this_cat'].'\');">'.icons($folder_new_unlocked, $lang['home']['open_cat'], "hspace=\"3\"").'</a>';
							}
							echo'<a href="index.php?mode=delete&type=c&c='.$c.'"  onclick="return confirm(\''.$lang['home']['you_are_sure_to_delete_this_cat'].'\');">'.icons($folder_new_delete, $lang['home']['delete_cat'], "hspace=\"3\"").'</a>
							</nobr></td>';
						}
					echo'
					</tr>';
					$forums = DBi::$con->query("SELECT * FROM ".$Prefix."FORUM WHERE CAT_ID = '$c' ORDER BY F_ORDER ASC") or die (DBi::$con->error); // forum mysql
           ...
           if ($f_level == 0 OR $f_level > 0 AND $f_level <= $Mlevel){
				 ...
				 // category while end
					...
					echo'
					<tr>
						<td class="cat_new" width="50%" td colspan="15"  td background="images/fia.png"class="cat_new"><nobr></nobr>
				      <div align="center"><font color = "#ffffff"><b>'.$cat_name.'</b></div></td>
     </tr>';
					
					echo'
					<tr>
						<td class="stats_p" width="50%"><nobr>'.$lang['active']['forum'].'</nobr></td>';
					
                                              if ($Mlevel == 4) {
                       echo '<td class="stats_p"><nobr><a href="index.php?mode=admin_svc&type=forumsorder">'.icons($icon_arrowup).icons($icon_arrowdown).'</a></nobr></td>';
}
						echo'<td class="stats_p" width="5%"><nobr>'.$lang['home']['topics'].'</nobr></td>
						<td class="stats_p" width="5%"><nobr>'.$lang['home']['posts'].'</nobr></td>
						<td class="stats_p"><nobr>'.icons($icon_group).'</nobr></td>
						<td class="stats_p"><nobr>'.$lang['home']['last_post'].'</nobr></td>				
						<td class="stats_p"><nobr>'.$lang['home']['monitors'].'</nobr></td>
	<td class="stats_p" width="30%" '.($Mlevel==3&&$CAN_CONROL_FORUMS==1?'colspan=2':'').'><nobr>'.$lang['home']['moderators'].'</nobr></td>';
						if ($Mlevel == 4) {
							echo'
							<td class="stats_p" vAlign="middle" align="middle"><nobr>
								<a href="index.php?mode=add_cat_forum&method=add&type=f&c='.$c.'">'.icons($folder_new, $lang['home']['add_new_forum'], "hspace=\"3\"").'</a>
								<a href="index.php?mode=add_cat_forum&method=edit&type=c&c='.$c.'">'.icons($folder_new_edit, $lang['home']['edit_cat'], "hspace=\"3\"").'</a>';
							if ($cat_status == 1) {
								echo'
...
echo'	
<tr>
<td class="cat_new" vAlign="top" colSpan="1">&nbsp;<b><a href="'.$site_name.'"><font color="white">'.$forum_title.'</font>
</tr>
<tr>';
...
$sql_cat_1 = DBi::$con->query("SELECT * FROM ".prefix."CATEGORY WHERE SITE_ID = '$Site_ID' OR SITE_ID = '0' ORDER BY CAT_ORDER ASC");
$num_cat_1 = mysqli_num_rows($sql_cat_1);
$x_cat_1 = 0;
while ($x_cat_1 < $num_cat_1) {
$cat_id_1 = mysqli_result($sql_cat_1, $x_cat_1, "CAT_ID");	
$cat_level_1 = mysqli_result($sql_cat_1, $x_cat_1, "CAT_LEVEL");	
$cat_hide_1 = mysqli_result($sql_cat_1, $x_cat_1, "CAT_HIDE");
$check_cat_login_1 = check_cat_login($cat_id_1);

$sql_forum_1 = DBi::$con->query("SELECT * FROM ".prefix."FORUM WHERE CAT_ID = '$cat_id_1' ORDER BY F_ORDER ASC");
$num_forum_1 = mysqli_num_rows($sql_forum_1);
$x_forum_1 = 0;
while ($x_forum_1 < $num_forum_1) {
$f_id_1 = mysqli_result($sql_forum_1, $x_forum_1, "FORUM_ID");	
$f_subject_1 = mysqli_result($sql_forum_1, $x_forum_1, "F_SUBJECT");	
$f_level_1 = mysqli_result($sql_forum_1, $x_forum_1, "F_LEVEL");	
$f_hide_1 = mysqli_result($sql_forum_1, $x_forum_1, "F_HIDE");
$check_forum_login_1 = check_forum_login($f_id_1);

if ($cat_level_1 == 0 OR $cat_level_1 > 0 AND $cat_level_1 <= $Mlevel) {
if ($cat_hide_1 == 0 OR $cat_hide_1 == 1 AND $check_cat_login_1 == 1) {
if ($f_level_1 == 0 OR $f_level_1 > 0 AND $f_level_1 <= $Mlevel){
if ($f_hide_1 == 0 OR $f_hide_1 == 1 AND $check_forum_login_1 == 1){
echo'
<a class="menu" style="font-size:9pt" href="'.$site_name2.'/index.php?mode=f&f='.$f_id_1.'">'.$f_subject_1.'</a>&nbsp;&nbsp;<font color="red">*</font>
';
}
}
}
