...
if (!empty($_POST[file_id])){
	...
	if (chk_mf_id($file_id) == 1){
		$mf_subject = mf("NAME", $file_id);
		DBi::$con->query("DELETE FROM ".$Prefix."MEDAL_FILES WHERE MF_ID = '$file_id' ") or die (DBi::$con->error);
...
function mf_files($id){
	...
	<tr class="normal">
		...
		<td class="list_small"><nobr><a href="index.php?mode=svc&method=add&svc=medals&f='.$f.'&m='.$id.'">'.$lang['others']['add_new_medal_here'].'</a></nobr></td>
...
<table width="100%">
	<tr>
		<td class="optionsbar_menus" width="100%"><nobr><font color="red" size="+1">'.$lang['others']['medal_files_f'].' '.forums("SUBJECT", $f).'</font></nobr></td>
	</tr>
</table>
<br>
...
<table dir="rtl" cellSpacing="0" cellPadding="0" border="0">
	<tr>
		...
		<table bgcolor="gray" class="grid" cellSpacing="0" cellPadding="0" width="100%" align="center" border="0">
			...
			$sql = DBi::$con->query("SELECT * FROM ".$Prefix."MEDAL_FILES WHERE FORUM_ID = '$f' ORDER BY MF_ID DESC") or die (DBi::$con->error);
			$num = mysqli_num_rows($sql);
			$x = 0;
			while ($x < $num) {
				$mf_id = mysqli_result($sql, $x, "MF_ID");
				mf_files($mf_id);
				$count = $count + 1;
			++$x;	
			}
...
if ($type == "upload"){
 ...
 $sql1 = DBi::$con->query("SELECT * FROM ".prefix."MEDAL_FILES WHERE FORUM_ID = '$f'");
...
$file_name = $f.'_'.$num1.'.'.file_extension($_FILES['up']['name']);
   ...
   {
      mkdir($image_folder.'medals/forum'.$f);     
   }
		if ($error != ""){
	                echo'<br>
					<center>
		...
		if ($error == ""){
			...
			$sql = "INSERT INTO ".$Prefix."MEDAL_FILES (MF_ID, FORUM_ID, ADDED, SUBJECT, DATE, NAME) VALUES (NULL, ";
			$sql .= " '$f', ";
			$sql .= " '$DBMemberID', ";
			$sql .= " '$img_url', ";
			$sql .= " '".time()."', ";
